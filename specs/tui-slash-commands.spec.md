...
### REQ-CMD-7 /model 呼出模型列表与切换
选中 `/model` → **不直接执行**，而是进入"模型选择模式"：在输入框上方呼出第二个
`SelectMenu`，列出 `availableModels`（每个模型项 `label` 为展示名、`hint` 为协议/model id）。
- ↑/↓ 移动高亮、Enter 选中切换、Esc 关闭回到普通输入（与命令菜单一致的交互）。
- 选中一个模型 → 调 `onSwitchModel(name)` 取得新 `Brain`，App 内部替换当前 brain；
  **对话历史保留**，下一条消息由新 brain 接收完整 history。
- 切换后清空输入、关闭菜单。当前激活模型在状态栏或 Header 标识。
- `availableModels` / `onSwitchModel` 为**可选 prop**：未传入时 `/model` 命令不出现或选中无副作用，
  保证不破坏既有测试与 mock-only 部署。
...
| REQ-CMD-7 | `slashCommands.test.ts`：注册表含 /model。`App.integration.test.tsx`：选中 /model 后帧出现 model 列表；选中某 model 触发 onSwitchModel spy 且 brain 被替换；Esc 关闭 model 列表 |

## 关联实现

- `src/platforms/tui/logic/selectMenu.ts`（`filterItems`/`moveSelection`/`visibleWindow`/`idleMenuState`）
- `src/platforms/tui/logic/slashCommands.ts`（`SLASH_COMMANDS`/`isCommandQuery`，含 `/model`）
- `src/platforms/tui/components/SelectMenu.tsx`（纯渲染上拉框，命令列表与模型列表复用）
- `src/platforms/tui/components/DynamicInput.tsx`（命令模式让渡导航键）
- `src/platforms/tui/index.tsx`（App 编排、命令分发、/exit→退出、/model→模型菜单+brain 切换）
- `src/platforms/tui/theme.ts`（`selected` 选中行色）
- `src/index.ts`（注入 `availableModels` + `onSwitchModel`，包装 `selectBrain`）
